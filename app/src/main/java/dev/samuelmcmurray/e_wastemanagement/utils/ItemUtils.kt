...
class ItemUtils {
    ...
    fun addBid(item: Item, companyName: String, price: Int): Boolean {
        val bidCollection = FirebaseFirestore.getInstance().collection("bids")
        val itemMap = hashMapOf(
            "name" to item.name,
            "UID" to item.userID,
            "type" to item.type,
            ...
            "id" to item.id,
            "model" to item.model,
            "description" to item.description + "::" + companyName + "::" + price
        )

        var success = false
        bidCollection.add(itemMap).addOnSuccessListener {
            Log.d(TAG, "onSuccess: ${item.id} added!")
            success = true
        }.addOnFailureListener {
            Log.d(TAG, "onFailure: ${item.id} failed to be added!")
    ...
    fun readBids(itemsCallback: ItemsCallback): Boolean {
        val bidCollection = FirebaseFirestore.getInstance().collection("bids")
        var success = false
        bidCollection.get().addOnSuccessListener { items ->
            if (items.isEmpty) {
